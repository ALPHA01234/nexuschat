...
function handleRegister() {
  ...
  const display = document.getElementById('regDisplay').value.trim() || username;
  const errEl = document.getElementById('registerError');
  errEl.textContent = '';

  if (!/^[a-zA-Z0-9_]{3,16}$/.test(username)) {
    errEl.textContent = 'Username must be 3-16 characters: letters, numbers, underscore only.';
    return;
  }
  ...
  const pfp = preview.dataset.image
...
function enterApp(username) {
  ...
  currentUserData = getUsers()[username];
